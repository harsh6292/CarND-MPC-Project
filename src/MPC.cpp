...
// Expected values of various variables that the solver should try to achieve
double ref_v = 30;
double ref_cte = 0;
double ref_epsi = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Minimize the reference variables error (CTE, epsi and velocity)
    // Give more weightage to CTE, epsi to reduce their error to zero
    for (int i = 0; i < N; i++) {
      //fg[0] += 17000 * CppAD::pow((vars[cte_start + i] - ref_cte), 2);
      //fg[0] += 2200 * CppAD::pow((vars[epsi_start + i] - ref_epsi), 2);
      fg[0] += 18 * CppAD::pow((vars[cte_start + i]), 2);
      fg[0] += 2 * CppAD::pow((vars[epsi_start + i]), 2);
      fg[0] += 0.5 * CppAD::pow((vars[v_start + i] - ref_v), 2);
    }


    // Minimze the orientation and acceleration
    for (int i = 0; i < N-1; i++) {
      fg[0] += 3 * CppAD::pow(vars[delta_start + i], 2);
      fg[0] += 2 * CppAD::pow(vars[a_start + i], 2);
    }

    // Smooth the orientation angle change and acceleration change errors
    for (int i = 0; i < N-2; i++) {
      fg[0] += 36000 * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += 110 * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set the upper and lower bound values for state (x, y, psi, y, cte, epsi) to 
  // max and min values as we don't care about their values
  for (int i =0; i < delta_start; i++) {
    vars_upperbound[i] = 1.0e19;
    vars_lowerbound[i] = -1.0e19;
  }

  // The delta value should be in range [-25, 25]
  for (int i = delta_start; i < a_start; i++) {
    vars_upperbound[i] = 0.436332;
    vars_lowerbound[i] = -0.436332;
  }

  // The accelaration values are in range [-1, 1]
  ...
  options += "Numeric max_cpu_time          40\n";
